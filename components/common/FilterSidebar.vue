<template>
  <div class="bg-white p-4 rounded-xl w-[400px]">
    ...
    <div class="border-b border-gray-200">
      <button class="flex justify-between items-center w-full py-5" @click="toggleSections">
        <h3 class="font-normal text-base text-gray-900">Разделы</h3>
        <i :class="{ 'transform rotate-180 text-red-500': sectionsOpen }"
          class="icon-chevron-down flex items-center justify-center w-5 h-5 transition-transform duration-200 text-gray-500"></i>
      </button>

      <div v-show="sectionsOpen" class="flex flex-col">
        <BaseCheckBox v-model="allSections" class="mb-2">Все разделы</BaseCheckBox>
        <div v-for="section in sections" :key="section.id" class="flex m-0 p-0 justify-between">
          <BaseCheckBox :submenu="section.hasSubmenu" v-model="section.checked">{{ section.name }}</BaseCheckBox>
        </div>

        <button @click="showMore = !showMore"
          class="text-sm font-normal flex text-red-500 gap-2 items-center mt-2 mb-5">
          <i :class="{ 'transform rotate-180': !showMore }"
            class="icon-chevron-down flex items-center justify-center w-5 h-5 transition-transform duration-200 text-red-500"></i>
          Еще +12
        </button>
      </div>
    </div>
    ...
    <div class="border-b border-gray-200">
      <button class="flex justify-between items-center w-full py-5" @click="toggleManufacturers">
        <h3 class="font-normal text-base text-gray-900">Производители</h3>
        <i :class="{ 'transform rotate-180 text-red-500': manufacturersOpen }"
          class="icon-chevron-down flex items-center justify-center w-5 h-5 transition-transform duration-200 text-gray-500"></i>
      </button>
    ...
    <div @click="discountedOnly = !discountedOnly"
    ...
    <div class="pt-5">
      <span class="font-normal text-base text-gray-900 block mb-1.5">Диапазон цены (сум)</span>
      <div class="flex items-center space-x-2">
        <div class="flex-1">
          <span class="text-sm text-gray-500 mb-1 block">от</span>
          <input type="number" v-model="priceFrom" class="w-full p-2 border border-gray-200 rounded-lg" />
        </div>
        <div class="flex-1">
          <span class="text-sm text-gray-500 mb-1 block">до</span>
          <input type="number" v-model="priceTo" class="w-full p-2 border border-gray-200 rounded-lg" />
        </div>
      </div>
    </div>
  </div>
